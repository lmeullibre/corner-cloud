...
class GuacamoleAdminAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if connections is None:
            return Response({"error": "Failed to retrieve connections."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class GenerateGuacamoleTokenViewExpert(APIView):
    ...
    @swagger_auto_schema(
        manual_parameters=[
            openapi.Parameter('game_id', openapi.IN_QUERY, description="ID of the game", type=openapi.TYPE_STRING)
        ]
    )
    def post(self, request, *args, **kwargs):
        game_id = request.GET.get('game_id')  # Fetching the game ID from query parameters
